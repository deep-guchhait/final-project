...
const FilterSidebar = () => {
    ...
    useEffect(() => {
      ...
      // {category: "Top Wear"} => params.category
  ...
  return (
    <div className='p-4'>
      ...
      {/**Category Filter */}
      <div className="mb-6">
        <label className='block text-gray-700 font-medium mb-2'>Ctegory</label>
        {categories.map((category) => (
          <div key={category} className='flex items-center mb-1'>
            <input type='radio' name='category' value={category} 
                   onChange={handleFilterChange} checked={filters.category === category} 
                   className='mr-2 h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-400' />
        ...
        <div className='flex flex-wrap gap-2'>
          {colors.map((color) => (
            <button key={color} name='color' value={color} 
                    ...
                    className={`w-8 h-8 rounded-full border border-gray-700 cursor-pointer transition hover:scale-105 ${filters.color === color ? "ring-2 ring-blue-500" : ""}`} 
        ...
        {sizes.map((size) => (
          <div key={size} className='flex items-center mb-1'>
            <input type="checkbox" name='size' value={size} 
                   ...
                   className='mr-2 h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-500' />
